...
- Every method on `spi.Provider` present (twelve today; count them against the file); a compile-time `_ spi.Provider = (*Provider)(nil)` assertion present, in a `var` line or a grouped block. The optional interfaces (`spi.PathSessionReader`, `spi.ProgressEnumerator`) are also defined in [pkg/spi/provider.go](../pkg/spi/provider.go); neither is required for SPI conformance. Check assertions for each one the provider implements.
...
- `ListAllAgentChatSessions` reads the originating cwd from inside the session; `spi.PathSessionReader` recommended if by-id lookup walks the store; `spi.ProgressEnumerator` recommended for live scan counts, using `spi.ScanSessionsInParallel` for a JSONL store (it walks `*.jsonl` only; other stores implement the enumeration themselves).
...
- `spi.SplitCommandLine`; `spi.EnsureResumeArgs` for subcommand-style resume; `spi.EnsureResumeFlagArgs` for flag-style resume, with the agent's supported flag names. The requested id replaces configured ids, bare or empty flags are filled, and the caller's slice is preserved.
