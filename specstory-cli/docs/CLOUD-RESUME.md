...
- **List (`GET …/{projectId}/sessions`)** — new opt-in **`?resumable=true`** param → filter `WHERE session_data_size > 0` (omitted = today's behavior, so the web app is untouched); return `metadata` (for the machine badge/filter); with `resumable=true` the server returns the **complete** set via internal paging (D21) — the `limit` param and its default (200) apply only to the web app's ungated path, which is unchanged (D5/D9/D21).
- **List projects (`GET /api/v1/projects`)** — reused as-is for the blended project list (D16).
- **Aligned search** — a **new endpoint** (own pathname → own rate-limit bucket, D18): build `to_tsquery('simple', …)` mirroring `ftsQuery` (implicit AND, phrase `<->`, last-token `:*`) against `exchanges.fts_simple` — constructive tokenization only (no raw input reaches the tsquery parser), empty token list short-circuits to zero hits, errors return empty rather than 500 (D18); filter `session_data_size > 0`; return **`ts_headline`** snippets delimited with STX/ETX to match local `snippet()`; sort by recency; uncapped via internal paging (D17/D18/D19/D21). Kept separate from the web app's `english` `searchSessions`.
...
Six **development chunks**, done in order — these are *work* phases, **not** releases. Each spans CLI + `specstory-sync` and ends with a **manual test/validation gate** before starting the next. Ordering follows the dependency graph: everything needs `SessionData` in the cloud (Chunk 1); resume establishes the reusable TUI plumbing (Chunk 2); search layers onto it (Chunk 3). Chunk 1 is done first so the corpus accumulates (and the back-catalog heals) while 2–3 are built. Chunks 4–6 refine what landed: 4 fixes the all-projects rollup's data quality; 5 adds direct session addressing (the `--session` flag and the web app's resume button); 6 removes the cloud read caps (D21 revision). Note: Chunks 2–3's work items below record the 500-cap as built — Chunk 6 supersedes those caps.
...
- **D29 — Three accepted URI forms, one parser.** (1) **Custom scheme** `specstory://projects/{projectID}/sessions/{sessionID}` — the **canonical form and what the web app's resume dialog copies (D36)**: host-free (no D30 host question by construction) and it reserves the door for a future OS protocol handler. (Parser note: `url.Parse` puts `projects` in `Host` for a custom scheme — rejoin host+path before matching.) (2) **Web permalink** `https://{host}/projects/{projectID}/sessions/{sessionID}` — accepted so a user can paste their browser URL bar directly; same path grammar; trailing slashes tolerated (matching the existing cloud-URL normalization) and path segments after the session id (e.g. `/chats/{chatId}`) ignored, so any session-page URL pastes cleanly. (3) **Bare session UUID** — shorthand, resolves local-first then cloud. ID shapes are validated up front for early, clear errors: project id is the `xxxx-xxxx-xxxx-xxxx` 16-hex-char form (`createHash`, `pkg/utils/project_identity.go:256`), session id is a UUID; both case-insensitive.
- **D30 — A pasted URL contributes IDs only; the Bearer token only ever goes to the *configured* cloud URL.** The CLI never adopts the permalink's host. If the permalink's host differs from the configured cloud host (default `https://cloud.specstory.com`, overridable via `--cloud-url`/config), fail with: "This link points at {host}, but the CLI is configured for {configured}. Pass --cloud-url https://{host} if this is intentional." Rationale: a crafted "resume link" must not exfiltrate the user's token to an attacker's host. Bare-UUID and `specstory://` forms carry no host, so no check applies.
- **D31 — Resolution order mirrors TUI selection.** (1) **Local index first**: look up the session id across agents in `sessions.db` (`deleted = 0`; a new store query — `GetSession`, `store.go:639`, is `(agent, session_id)`-keyed and can't do this) — found means a local in-place resume using the row's `agent`/`origin_cwd`, exactly like D13's guard (this also makes a pasted *cloud* permalink resume locally, offline-capable, when the session exists here). In the near-impossible event two agents share a session UUID, prefer the most recent `updated_at`. (2) **Else cloud**, gated by D10 eligibility surfaced as actionable errors instead of nudges (not logged in → "Log into SpecStory Cloud (specstory login) to resume sessions from your other machines."; 403 → the existing "requires SpecStory Pro" mapping in `FetchSessionData`): with a project id in hand, fetch directly; a bare UUID checks the current project's cloud list (`ListCloudSessions`) then the all-projects `?resumable=true` listing (D24 embeds every resumable session, one request) to discover the owning project. (3) **Else** a "session not found" error naming where it looked. D12's `schemaVersion` guard and cross-project resume semantics are inherited unchanged from the shared path.
- **D32 — The `[agent]` positional stays the target override; omitting it opens the TUI directly at the target picker.** `resume <agent> --session <uri>` is fully non-interactive (plan built directly, no TUI — the `presetTo` short-circuit's equivalent). `resume --session <uri>` launches the TUI in `modeTarget` with the resolved session pinned as `chosen`, default highlight logic unchanged (last-resumed agent, else the session's own agent — `defaultTargetIndex`, `session_tui.go:1002`). Consequence: the web-copied command omits the agent — the terminal owns target choice, so the web dialog needs **no** provider picker.
- **D33 — Web: a resume icon button in `SessionHeader`, between Download and Delete.** Owner-only (like share/delete — the CLI's fetch is owner-validated server-side anyway) and visible under the existing `resume` feature flag (`useFlag`). **Enabled only when the session is resumable** (`sessionDataSize > 0`); otherwise disabled with a tooltip ("This session predates resume support — re-sync it from its machine with a current CLI to enable."). The wire already carries `sessionDataSize` (`SessionDetailSchema`, `shared/types.ts:159`; `toSessionSummary`, `worker/supabase.ts:352`) but the frontend drops it — it must be added to `SessionDetail` (`services/api.ts:52-84`) and threaded through `SessionView`'s hand-built `sessionData` prop (`SessionView.tsx:102-117`).
- **D34 — Click: entitled (`can("resume")`) → command dialog; not entitled → navigate to `/resume`.** The `/resume` hub (`App.tsx:321`) is the feature's explanation/upsell page, so it's the richer sell — an intentional divergence from `ResumeButton.tsx`'s inline `promptUpgrade("resume")` pattern.
- **D35 — The dialog auto-copies on open, and shows the command with a copy-again affordance.** Pattern to follow: `ProviderPicker.tsx` (already a Dialog that builds a `specstory resume` command with a copied-state Copy button) plus the shared `copyToClipboard` util (`lib/utils/share.ts:32`, clipboard API + fallback). Content: brief explanation — pasting this in a terminal resumes the session on that machine via the **SpecStory CLI**, which must be installed (link install docs) and logged into SpecStory Cloud (`specstory login`) — then the command in a code block with the Copy button. No agent picker (D32).
- **D36 — The web copies the `specstory://` form, appending `--cloud-url` on non-production origins.** The command is `specstory resume --session specstory://projects/{projectId}/sessions/{sessionId}` — the canonical, host-free URI (D29), deliberately *not* the page URL, so the copied command carries no host at all and D30's host check never enters the picture for the product-emitted flow. Because the URI is host-free, non-production deployments need the target host expressed some other way: append ` --cloud-url {window.location.origin}` when the origin differs from the production constant `https://cloud.specstory.com`, so cloud-dev/self-hosted copies work with zero manual flags while production commands stay clean. (The frontend has no environment config for this today — `API_BASE = window.location.origin`, `services/api.ts:188` — hence the compare-to-prod-constant approach.) The https permalink form remains supported purely for hand-pasted browser URLs.

#### Work items

- **CLI (`specstory-cli`):**
  - Add the `--session` flag in `CreateResumeCommand` (`resume.go:70`) — deliberately not in `registerResumeLaunchFlags`, so `search` is untouched (D28).
  - Session-URI parser: three forms (D29), shape validation, host extraction for the D30 check; table-driven unit tests (valid/invalid shapes, trailing slash, `/chats/…` suffix, case-insensitivity, `specstory://` host/path rejoin).
  - `pkg/sessionindex`: new lookup by bare `session_id` across agents (most-recent-first for the collision tiebreak, D31).
  - Resolution (D31): local-first via the new lookup + `resumeSourceForSession` (`session_tui_browser.go:186`) semantics; cloud via `FetchSessionData` (direct) or `ListCloudSessions`/`ListCloudProjects` discovery (bare UUID); convert cloud hits with the existing `cloudToSessions` (`session_tui_cloud.go:266`) so a cloud-resolved session is the same `sessionindex.Session` row the TUI produces.
  - Entry wiring (D32): `[agent]` given → build the `resumePlan` directly and call `launchResume`; omitted → launch the session TUI starting in `modeTarget` with `chosen` pinned (skipping the browse step).
  - Error paths: host mismatch (D30), not found, not-logged-in / not-Pro (D31 wording), newer `schemaVersion` (D12, inherited).
- **Web (`specstory-sync` frontend; no worker changes):**
  - `services/api.ts`: add `sessionDataSize?: number | null` to `SessionDetail`; `SessionView.tsx`: thread it into the `sessionData` object passed to the header (D33).
  - `SessionHeader.tsx`: resume icon button between the Download (497-517) and Delete (518-526) buttons; `isOwner` + `useFlag("resume")` + resumable gating with the disabled-state tooltip (D33); entitled click opens the dialog, non-entitled click navigates to `/resume` (D34).
  - New resume-command dialog component (sibling of `ProviderPicker.tsx`): auto-copy on open, command code block, copy-again button, explanatory text (D35); command construction per D36 (origin vs prod constant).
- **Server:** none — the SessionData read (D8), the `?resumable=true` listings (D5/D24), and the entitlement guard (D10) already cover everything this chunk calls.
- **Validation:**
  - Paste the web-copied command (`specstory://` form) for a cloud-only session → target picker appears pinned to that session; choosing an agent fetches, reconstructs, and launches.
  - `resume claude --session <uri>` → fully non-interactive, no TUI.
  - Bare UUID of a local session → resumes locally, in place, offline.
  - A cloud session URI (either form) for a session that also exists locally → resumes the local copy (D13/D31 guard), works offline.
  - Bare UUID of a cloud-only session from another machine and another project → discovered via the all-projects listing, resumes.
  - A hand-pasted browser permalink (`https://…/projects/{pid}/sessions/{sid}`, with and without trailing slash / `/chats/…` suffix) resolves identically to the `specstory://` form.
