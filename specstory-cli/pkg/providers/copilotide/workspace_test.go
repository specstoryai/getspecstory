...
func TestUriToPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "file URI with encoded colon in path",
			uri:      "file:///h%3A/General%20Workspace.code-workspace",
			wantPath: "/h:/General Workspace.code-workspace",
		},
...
func TestUriToPath_WindowsPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Windows mapped drive with code-workspace file",
			uri:      "file:///h%3A/General%20Workspace.code-workspace",
			wantPath: "h:\\General Workspace.code-workspace",
		},
	}
...
func TestCollectCodeWorkspaceFolders(t *testing.T) {
	...
	// Resolve symlinks so the canonical path matches what GetCanonicalPath returns
